...
This wrapper intentionally sets the Content-Type and Accept headers to \`application/ld+json\` for all requests. Exceptions: \`queryBatch\` and \`temporalQueryBatch\` are still using \`application/json\`, reason being that [Stellio doesn't support \`@context\` property in query batch and temporal query batch](https://github.com/stellio-hub/stellio-context-broker/issues/1809). Thus the SDK wrapper sets the Content-Type and Accept headers to \`application/json\` for those two requests.
